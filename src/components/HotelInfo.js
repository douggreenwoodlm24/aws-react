...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async () => {
    ...
    const resp = await fetch(
      "https://37yhk6o9fi.execute-api.eu-west-2.amazonaws.com/Production/accessibilities"
    );
  ...
  const loadServicesData = async () => {
    // Query the API Gateway
    const resp = await fetch(
      "https://37yhk6o9fi.execute-api.eu-west-2.amazonaws.com/Production/services"
    );
    let jsonData = await resp.json();
